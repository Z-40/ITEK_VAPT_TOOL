  ...
  return (
    <nav className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-black/70 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <button onClick={() => handleNavClick("landing")} className="flex items-center gap-2.5 bg-transparent border-none outline-none cursor-pointer">
        ...
        <div className="border-t border-white/10 bg-black/95 px-6 py-5 md:hidden">
          <div className="flex flex-col gap-4 text-sm text-gray-300">
            ...
            <div className="mt-2 flex flex-col gap-3 border-t border-white/10 pt-4">
              <button 
                onClick={() => handleNavClick("login")}
                className="rounded-lg border border-white/15 px-4 py-2.5 text-sm font-medium text-gray-200"
              >
                Log in
              </button>
              <button 
                onClick={() => handleNavClick("signup")}
                className="rounded-lg bg-gradient-to-r from-emerald-400 to-cyan-400 px-4 py-2.5 text-sm font-semibold text-black"
              >
                Sign up
              </button>
            </div>
  ...
  return (
    <section id="top" className="relative overflow-hidden px-6 pb-24 pt-40">
      ...
      <div className="relative z-10 mx-auto max-w-4xl text-center">
        <span className="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/5 px-4 py-1.5 font-mono text-xs uppercase tracking-[0.2em] text-emerald-400">
          Offensive Security Platform
        </span>

        <h1 className="mt-8 font-mono text-5xl font-bold leading-[1.05] tracking-tight text-white sm:text-6xl lg:text-7xl">
          Attack your own
          <br />
        ...
        <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-gray-400 sm:text-xl">
          ITEK automates recon, exploitation, and CVE correlation across your entire attack
          surface — so the next intrusion attempt finds nothing left to take.
        </p>

        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <button 
            onClick={() => setView("signup")}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-400 to-cyan-400 px-8 py-3.5 text-sm font-semibold text-black transition hover:brightness-110 focus:outline-none"
          >
            Start a free assessment
          </button>
          <a
            href="#demo"
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 px-8 py-3.5 text-sm font-semibold text-gray-200 transition hover:border-emerald-400/50 hover:text-white focus:outline-none"
          >
            Watch a live engagement →
          </a>
        </div>
      </div>
    </section>
  ...
  return (
    <section id="capabilities" className="mx-auto max-w-7xl px-6 py-24">
      <div className="mb-14 max-w-2xl">
        <span className="font-mono text-xs uppercase tracking-widest text-emerald-400">Capabilities</span>
        <h2 className="mt-3 font-mono text-3xl font-bold tracking-tight text-white sm:text-4xl">
          Five stages. One pipeline.
        </h2>
        <p className="mt-4 text-gray-400">
          Every engagement runs the same disciplined sequence — from first DNS lookup to confirmed exploit.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {FEATURES.map((f) => (
          <div
            key={f.n}
            className="group relative rounded-2xl border border-white/10 bg-white/[0.02] p-6 transition-all duration-300 hover:border-emerald-400/40 hover:bg-white/[0.04]"
          >
            <div className="mb-5 flex items-center justify-between">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-400 group-hover:bg-emerald-500/20">
                <f.icon className="h-5 w-5" />
              </div>
              <span className="font-mono text-xs text-gray-600">{f.n}</span>
            </div>
            <p className="mb-2 truncate font-mono text-[11px] text-cyan-400/80">$ {f.cmd}</p>
...
function TerminalDemo() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setVisibleCount((c) => (c >= SCAN_LINES.length + 4 ? 0 : c + 1));
    }, 450);
  ...
  return (
    <section id="demo" className="bg-gradient-to-b from-transparent via-emerald-500/[0.03] to-transparent px-6 py-24">
      <div className="mx-auto mb-12 max-w-3xl text-center">
        <span className="font-mono text-xs uppercase tracking-widest text-cyan-400">Live engagement preview</span>
        <h2 className="mt-3 font-mono text-3xl font-bold tracking-tight text-white sm:text-4xl">Watch ITEK work in real time</h2>
      </div>
      ...
      <div className="mx-auto max-w-3xl overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/80 shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-2 border-b border-white/10 bg-white/[0.02] px-4 py-3">
          ...
          <span className="mx-auto font-mono text-xs text-gray-500">itek — engagement/acme-corp.io</span>
        </div>
        ...
        <div className="min-h-[360px] p-6 font-mono text-[13px] leading-relaxed sm:text-sm">
          {shown.map((line, i) => {
            if (line.type === "cmd") {
              return (
                <p key={i} className="text-gray-200">
                  <span className="text-emerald-400">itek@core</span><span className="text-gray-500">:~$ </span>{line.text}
                </p>
              );
            }
            if (line.type === "log") {
            ...
            return (
              <p key={i} className="flex flex-wrap items-baseline gap-2 py-0.5">
                <span className={`shrink-0 rounded border px-1.5 py-0.5 text-[10px] font-bold tracking-wider ${SEVERITY_STYLES[line.type]}`}>
    ...
    <section className="px-6 py-20">
      <div className="relative mx-auto max-w-5xl overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-emerald-500/10 via-transparent to-cyan-500/10 p-10 text-center sm:p-14">
        <h2 className="relative font-mono text-3xl font-bold tracking-tight text-white sm:text-4xl">
          Your attack surface is already being scanned.
        </h2>
        <button 
          onClick={() => setView("signup")}
          className="relative mt-8 inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-400 to-cyan-400 px-8 py-3.5 text-sm font-semibold text-black transition hover:brightness-110 focus:outline-none"
        >
    ...
    <section className="relative flex min-h-[calc(100vh-4rem)] items-center justify-center px-6 pt-24 pb-12">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-zinc-950/70 p-8 shadow-2xl backdrop-blur-xl">
        <div className="mb-8 text-center">
          ...
          <h2 className="mt-2 font-mono text-2xl font-bold tracking-tight text-white">
    ...
    <footer className="border-t border-white/10 px-6 py-10">
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 text-sm text-gray-500 sm:flex-row">
        <div className="flex items-center gap-2.5">
          <span className="flex h-6 w-6 items-center justify-center rounded bg-gradient-to-br from-emerald-400 to-cyan-400 font-mono text-xs font-bold text-black">I</span>
          <span className="font-mono font-semibold text-gray-300">ITEK</span>
        </div>
...
export default function App() {
  // Check browser URL directly on cold load
  ...
  useEffect(() => {
    // Listens to browser Back/Forward navigation triggers
  ...
  // Updates path cleanly (/login, /signup, /) without full page refresh
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-black font-sans text-gray-100 antialiased selection:bg-emerald-500/30">
      <Navbar setView={navigateTo} view={view} />
      ...
      {view === "landing" ? (
        ...
        <AuthPage view={view} setView={navigateTo} />
      )}
