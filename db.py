...
def create_user(email: str, username: str, password: str) -> str:
    ...
    with get_cursor(commit=True) as cur:
        ...
        cur.execute(
            ...
            "verification_token, verification_token_expires) VALUES (%s, %s, %s, 0, %s, %s)",
...
def verify_email_token(token: str) -> str:
    """Consumes a verification token. Returns 'ok', 'expired', or 'invalid'.
    ...
    with get_cursor(commit=True) as cur:
        ...
        if user["email_verified"]:
            return "ok"  # already verified (e.g. link clicked twice) -- treat as success
        ...
        cur.execute(
            ...
            "verification_token_expires=NULL WHERE id=%s", (user["id"],),
        )
    return "ok"
